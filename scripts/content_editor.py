...
# Helpers
# ---------------------------------------------------------------------------
def build_system_blocks(guidelines_text: str) -> list[dict]:
    """Build system prompt as structured blocks with cache_control.

    Block 1: Editor instructions (static across all clients)
    Block 2: Brand guidelines (static per client — cached)

    The same blocks are reused across all 3 API calls so the guidelines
    are written to cache once and read from cache on calls 2 and 3.
    """
    return [
        {
            "type": "text",
...
def step2_violation_review(
    ...
    system_blocks = build_system_blocks(guidelines_text)
    user_prompt = (
    ...
    log_cache_stats(response, "violation_review")
...
def step3_annotate(
    ...
    log_cache_stats(response, "annotate")
...
def step4_clean_version(
    ...
    log_cache_stats(response, "clean_version")
    return response.content[0].text
def main():
